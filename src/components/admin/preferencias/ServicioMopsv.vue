      ...
      <v-card-text>
        ...
        >
          <template slot="buttons">
            <v-tooltip bottom v-if="$store.state.permissions['serviciosMopsv:create']">
          ...
          <template slot="items" slot-scope="items">
            <td class="nowrap">
              <v-tooltip bottom v-if="$store.state.permissions['serviciosMopsv:update']">
                <v-btn
                  icon
                  slot="activator"
                  @click.native="editItem(items.item.id, 'servicioMopsv', dataGraphql)">
                  <v-icon>edit</v-icon>
                </v-btn>
                <span>Editar registro</span>
              </v-tooltip>
              <v-tooltip bottom v-if="$store.state.permissions['serviciosMopsv:delete']">
                <v-btn
                  icon
                  slot="activator"
                  @click.native="deleteItem(items.item.id, 'servicioMopsv')">
            ...
            <td>
              <v-tooltip bottom v-if="$store.state.permissions['serviciosMopsv:update']">
                <v-switch
                  ...
                  value="ACTIVE"
                  @change="changeActive(items.item, items.item.id, 'servicioMopsv', 'EditServicioMopsv')"
                  hide-details
                  slot="activator"
      ...
      if (this.$refs.form.validate()) {
        ...
        if (data.id) {
          ...
          this.$service.graphql({
            query: `
              mutation edit($id: Int!, $servicio: EditServicioMopsv!) {
                servicioMopsvEdit(id: $id, servicioMopsv: $servicio) {
                  id
                }
              }
            `,
          ...
          this.$service.graphql({
            query: `
              mutation add($servicio: NewServicioMopsv!) {
                servicioMopsvAdd(servicioMopsv: $servicio) {
                  id
                }
              }
            `,
