        ...
        <v-container grid-list-md class="menu-person">
          <v-layout row wrap>
            ...
            <v-flex xs9>
              ...
              <v-icon>person_pin</v-icon> {{ (user.rol + '').replace(/_/gi, ' ') }} <br>
              <v-icon>location_city</v-icon>{{ user.entidad }}
              <em v-if="$util.isRol('FELCN_ADMIN')"><br>
              <v-icon>done_all</v-icon>{{ user.id_aeropuerto ? user.id_aeropuerto : 'Sin aeropuerto asignado'}}
              </em>
            </v-flex>
          </v-layout>
        </v-container>
...
.menu-person {
  ...
  background-color: lighten($bgSidenav, 7%);
