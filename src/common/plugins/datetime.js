    ...
    const DatetimeService = {
      ...
      format (date, format, utc = true) {
        if (date) {
          let d = new Date(date);
          if (this.isDate(d)) {
            return this.replace(d, format || this.formatDate, utc);
          }
          return date;
        }
      },
      ...
      replace (date, format, utc = true) {
        ...
        return Util.replace(format,
          ...
          [
            ...
            utc ? ((date.getUTCDate() < 10 ? '0' : '') + date.getUTCDate()) : ((date.getDate() < 10 ? '0' : '') + date.getDate()),
            utc ? (monthLiteral ? this.months[date.getUTCMonth()] : ((date.getUTCMonth() + 1 < 10 ? '0' : '') + (date.getUTCMonth() + 1))) : (monthLiteral ? this.months[date.getMonth()] : ((date.getMonth() + 1 < 10 ? '0' : '') + (date.getMonth() + 1))),
